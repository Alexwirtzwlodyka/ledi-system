...
final class AdjuntoController
{
    ...
    public function index(array $query): array { return $this->service->listByEscribano((int) ($query['escribano_id'] ?? 0)); }
    public function store(array $payload): array { return $this->service->upload((int) ($payload['escribano_id'] ?? 0), (string) ($payload['filename'] ?? ''), $this->normalizeContent($payload['content'] ?? ''), (int) ($payload['actor_user_id'] ?? 0), $payload['_actor'] ?? []); }
