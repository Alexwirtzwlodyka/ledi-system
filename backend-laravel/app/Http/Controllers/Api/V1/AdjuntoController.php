...
final class AdjuntoController
{
    ...
    public function update(array $payload): array { return $this->service->update((int) ($payload['adjunto_id'] ?? 0), (string) ($payload['filename'] ?? ''), $this->normalizeContent($payload['content'] ?? ''), (string) ($payload['content_encoding'] ?? 'plain'), (int) ($payload['actor_user_id'] ?? 0), $payload['_actor'] ?? []); }
